...
async function main(){
  ...
  // products (40)
  const products = [];
  const tags = ['sativa','indica','hybrid','edible','vape','topical','organic'];
  for(let i=0;i<40;i++){
    ...
    const p = await prisma.product.upsert({
      ...
      update:{name, priceCents: randInt(1000,30000), thc: parseFloat((Math.random()*30).toFixed(2)), cbd: parseFloat((Math.random()*10).toFixed(2)), tags: [tags[i%tags.length]]},
      create:{tenantId:tenant.id, name, slug, priceCents: randInt(1000,30000), thc: parseFloat((Math.random()*30).toFixed(2)), cbd: parseFloat((Math.random()*10).toFixed(2)), tags: [tags[i%tags.length]]}
    });
    products.push(p);
    // add one media
    await prisma.productMedia.upsert({
      where:{id: p.id},
      update:{url:`https://picsum.photos/seed/${slug}/600/400`, mime:'image/jpeg'},
      create:{id: p.id, productId: p.id, url:`https://picsum.photos/seed/${slug}/600/400`, mime:'image/jpeg'}
    });
  }
  ...
  for(let i=0;i<15;i++){
    ...
    await prisma.article.upsert({
      ...
      update:{title:`Demo Article ${i+1}`, body:'Sample content', published:true, publishedAt: new Date()},
      create:{tenantId:t.id, title:`Demo Article ${i+1}`, slug, body:'Sample content', published:true, publishedAt: new Date()}
    });
  }
  ...
  for(let i=0;i<5;i++){
    ...
    await prisma.deal.upsert({
      ...
      create:{id:`deal-${t.slug}-${i}`, tenantId:t.id, title:`Demo Deal ${i+1}`, description:'Demo deal', active:true}
    });
  }
  ...
  for(let tier=1;tier<=3;tier++){
    ...
    await prisma.award.upsert({
      ...
      create:{id:`award-${t.slug}-${tier}`, tenantId:t.id, title:`Award Tier ${tier}`, tier}
    });
  }
  ...
  for(let v=1;v<=3;v++){
    ...
    await prisma.legalDocument.upsert({
      ...
      create:{id:`legal-${t.slug}-${v}`, tenantId:t.id, docType:'terms', version:v, body:`Terms v${v}`}
    });
  }
  ...
  for(let i=0;i<30;i++){
    const t = tenantRecords[i%tenantRecords.length];
    await prisma.journalEntry.create({data:{tenantId:t.id, content:`Journal entry ${i+1}`, source:'seed'}});
  }

  // analytics noise
  for(let i=0;i<200;i++){
    const t = tenantRecords[i%tenantRecords.length];
    await prisma.analyticsEvent.create({data:{tenantId:t.id, type:'view', payload:{path:'/home', ts:Date.now()}}});
  }
