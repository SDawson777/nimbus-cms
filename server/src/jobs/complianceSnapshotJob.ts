...
export async function runComplianceSnapshotOnce() {
  ...
  const orgs = (await fetchCMS('*[_type=="organization"]{slug}', {})) as Array<{
    slug?: string;
  }>;
  const brands = (await fetchCMS('*[_type=="brand"]{slug}', {})) as Array<{
    slug?: string;
  }>;
  ...
  await client
    .create(globalDocHistory)
    .catch(() => client.createOrReplace(globalDocHistory));
  const globalLatest = {
  ...
  for (const o of orgs || []) {
    ...
    if (!orgSlug) continue;
    const results = await computeCompliance(
    ...
    try {
      ...
      const monitorDoc: any = {
        ...
        _type: "complianceMonitor",
        lastRuns: [
          { scope: `org:${orgSlug}`, snapshotId: latestDoc._id, ts: nowTs },
        ],
      };
      await client.createOrReplace(monitorDoc);
    } catch (e) {
  ...
  for (const b of brands || []) {
    ...
    try {
      ...
      const monitorDoc: any = {
        ...
        lastRuns: [
          {
            scope: `brand:${brandSlug}`,
            snapshotId: latestDoc._id,
            ts: nowTs,
            brandSlug,
          },
        ],
...
export function startComplianceScheduler(intervalMs?: number) {
  const ms =
    intervalMs ||
    Number(process.env.COMPLIANCE_SNAPSHOT_INTERVAL_MS || 1000 * 60 * 60);
  // fire and schedule
  ...
  const id = setInterval(
    () =>
      runComplianceSnapshotOnce().catch((e) =>
        logger.error("compliance.snapshot.failed", e),
      ),
    ms,
  );
