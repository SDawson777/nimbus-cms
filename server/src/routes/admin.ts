...
function resolveScopedBrand(
  ...
  if (!canAccessBrand(admin, targetBrand))
...
const OVERVIEW_CACHE_TTL = Number(
  process.env.ANALYTICS_OVERVIEW_CACHE_TTL_MS || 30000,
);
const complianceOverviewCache: Map<string, { ts: number; data: any }> =
  new Map();
...
async function readAnalyticsOverviewCacheDoc(
  cacheKey: string,
...
async function computeAnalyticsOverview(
  ...
  if (!forceRefresh) {
    ...
    if (cached && now - cached.ts < OVERVIEW_CACHE_TTL) {
      const refreshedAt =
        cached.refreshedAt || new Date(cached.ts).toISOString();
      return { payload: cached.data, cacheMode: "HIT", refreshedAt };
    }
    const persisted = await readPersistentAnalyticsOverviewCache(
      cacheKey,
      OVERVIEW_CACHE_TTL,
    );
    if (persisted) {
  ...
  try {
    settings = await fetchCMS(
      '*[_type=="analyticsSettings" && orgSlug == $org][0]',
      { org: orgSlug },
    );
  } catch (err) {
  ...
  const windowDays = Math.max(
    ...
    resolveNumber(
      [
        normalizedQuery.windowDays,
        settings?.windowDays,
        process.env.ANALYTICS_WINDOW_DAYS,
      ],
      30,
    ),
  );
  const recentDaysRaw = resolveNumber(
    [
      normalizedQuery.recentDays,
      settings?.recentDays,
      process.env.ANALYTICS_RECENT_DAYS,
    ],
    7,
  );
  ...
  const startHistoric = new Date(
    Date.now() - 1000 * 60 * 60 * 24 * windowDays,
  ).toISOString();
  ...
  const [topArticles, topFaqs, topProducts, productsForDemand, storeRows] =
    await Promise.all([
  ...
  ((productsForDemand as any[]) || []).forEach((row) => {
    ...
    if (!bySlug[slug])
      bySlug[slug] = { views: 0, clicks: 0, recentViews: 0, recentClicks: 0 };
  ...
  const groupedSeries: Record<
    string,
    Record<string, { views: number; clickThroughs: number }>
  > = {};
  ((productsForDemand as any[]) || []).forEach((row) => {
    const slug = row.contentSlug;
    const date = String(row.date).slice(0, 10);
    if (!groupedSeries[slug]) groupedSeries[slug] = {};
    if (!groupedSeries[slug][date])
      groupedSeries[slug][date] = { views: 0, clickThroughs: 0 };
  ...
  const slugsForSeries = ((topProducts as any[]) || []).map(
    (p: any) => p.contentSlug,
  );
  ...
  const storeEngagement = Object.keys(storeMap).map((k) => ({
    storeSlug: k,
    ...storeMap[k],
  }));
...
adminRouter.post(
  ...
  async (req, res) => {
    try {
      const admin = (req as any).admin;
      const bodyQuery = normalizeQueryParams(
      ...
      const result = await computeAnalyticsOverview(admin, mergedQuery, {
        forceRefresh: true,
      });
      res.json({
        ok: true,
...
async function readPersistentAnalyticsOverviewCache(
  cacheKey: string,
  ttlMs: number,
) {
...
async function writePersistentAnalyticsOverviewCache(
  cacheKey: string,
  data: any,
  ts?: string,
) {
...
function buildComplianceCacheKey(
  admin: any,
  types: string[],
...
function invalidateComplianceCache(scope: {
  org?: string | null;
  brand?: string | null;
}) {
...
adminRouter.get("/products", requireRole("EDITOR"), async (req, res) => {
  ...
  const includeRecalled =
    String((req.query as any).includeRecalled || "").toLowerCase() === "true";
  const admin = (req as any).admin;
  ...
  try {
    ...
    const mapped = (items || []).map((p: any) => {
      ...
      if (p.image && (p.image.url || p.image.alt))
        out.image = { url: p.image.url, alt: p.image.alt };
    ...
    const filtered = includeRecalled
      ? mapped
      : mapped.filter((m) => !m.isRecalled);
...
adminRouter.post(
  ...
  async (req, res) => {
    try {
      const admin = (req as any).admin;
      const { id } = z
        .object({ id: z.string().min(1, "MISSING_ID") })
        .parse(req.params || {});
      const body = z
        .object({
      ...
      const storeSlugs = Array.isArray(prev?.stores)
        ? prev.stores.filter(Boolean)
        : [];
      ...
      const previousState = {
        isRecalled: !!prev?.isRecalled,
        recallReason: prev?.recallReason || null,
      };

      const patch = client.patch(id);
      if (typeof isRecalled !== "undefined")
        patch.set({ isRecalled: !!isRecalled });
      if (typeof recallReason !== "undefined")
        patch.set({ recallReason: recallReason || null });
      ...
      if (err instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: "INVALID_RECALL_PAYLOAD", details: err.issues });
      }
...
adminRouter.post("/theme", requireRole("EDITOR"), async (req, res) => {
  ...
  try {
    ...
    if (
      schema.store &&
      !ensureStoreScope(res, admin, schema.store, schema.brand)
    )
      return;
    ...
    const brandId = await client.fetch(
      '*[_type=="brand" && slug.current==$b][0]._id',
      {
        b: schema.brand,
      },
    );
    ...
    if (schema.store) {
      storeId = await client.fetch(
        '*[_type=="store" && slug.current==$s][0]._id',
        {
          s: schema.store,
        },
      );
...
adminRouter.post("/theme/config", requireRole("EDITOR"), async (req, res) => {
  try {
    ...
    if (
      schema.store &&
      !ensureStoreScope(res, admin, schema.store, schema.brand || null)
    )
      return;
    ...
    if (schema.logoAssetId) {
      doc.logo = {
        _type: "image",
        asset: { _type: "reference", _ref: schema.logoAssetId },
      };
...
adminRouter.delete(
  ...
  async (req, res) => {
    try {
      ...
      if (!ensureStoreScope(res, admin, doc?.store || null, doc?.brand || null))
        return;
      const client = createWriteClient();
      // @ts-ignore
...
adminRouter.post("/theme/preview", requireRole("EDITOR"), async (req, res) => {
  try {
    ...
    if (e && (e as any).errors)
      return res
        .status(400)
        .json({ error: "INVALID", details: (e as any).errors });
    ...
    try {
      ...
      if (cached && Date.now() - cached.ts < OVERVIEW_CACHE_TTL) {
        ...
        if (out.topProducts && perPage)
          out.topProducts = out.topProducts.slice(
            page * perPage,
            page * perPage + perPage,
          );
        if (out.productSeries && perPage)
          out.productSeries = out.productSeries.slice(
            page * perPage,
            page * perPage + perPage,
          );
      ...
      const [topArticles, topFaqs, topProducts, productsForDemand] =
        await Promise.all([
          fetchCMS(topArticlesQ, params),
          fetchCMS(topFaqsQ, params),
          fetchCMS(topProductsQ, params),
      ...
      try {
        ...
        if (settings) {
          ...
          if (!qAny.windowDays && settings.windowDays)
            qAny.windowDays = settings.windowDays;
          if (!qAny.recentDays && settings.recentDays)
            qAny.recentDays = settings.recentDays;
          if (!qAny.wRecentClicks && settings.wRecentClicks)
            qAny.wRecentClicks = settings.wRecentClicks;
          if (!qAny.wRecentViews && settings.wRecentViews)
            qAny.wRecentViews = settings.wRecentViews;
      ...
      const storeQ = `*[_type==\"contentMetricDaily\" ${filter} && defined(storeSlug) && date >= $startHistoric]{storeSlug, date, views, clickThroughs}`;
      const storeRows: any[] =
        (await fetchCMS(storeQ, { ...params, startHistoric })) || [];
      const storeMap: Record<string, { views: number; clickThroughs: number }> =
        {};
      ...
      const recentCutoff = new Date(
        Date.now() - 1000 * 60 * 60 * 24 * recentDays,
      );
      ((productsForDemand as any[]) || []).forEach((row) => {
        const slug = row.contentSlug;
        if (!bySlug[slug])
      ...
      const wRecentClicks = Number(
        (req.query as any).wRecentClicks ||
          process.env.ANALYTICS_WEIGHT_RECENT_CLICKS ||
          2.5,
      );
      const wRecentViews = Number(
        (req.query as any).wRecentViews ||
          process.env.ANALYTICS_WEIGHT_RECENT_VIEWS ||
          0.2,
      );
      const wHistoricClicks = Number(
        (req.query as any).wHistoricClicks ||
          process.env.ANALYTICS_WEIGHT_HISTORIC_CLICKS ||
          1,
      );
      const wHistoricViews = Number(
        (req.query as any).wHistoricViews ||
          process.env.ANALYTICS_WEIGHT_HISTORIC_VIEWS ||
          0.05,
      );
      ...
      const productDemand = Object.keys(bySlug).map((slug) => {
        ...
        const demandScore =
          ...
          (s.views - s.recentViews) * wHistoricViews;
        let status = "lowEngagement";
        const risingThreshold = Number(
          process.env.ANALYTICS_THRESHOLD_RISING || 200,
        );
        const steadyThreshold = Number(
          process.env.ANALYTICS_THRESHOLD_STEADY || 40,
        );
        const fallingThreshold = Number(
          process.env.ANALYTICS_THRESHOLD_FALLING || 10,
        );
      ...
      rows.forEach((r) => {
        ...
        if (!grouped[slug][date])
          grouped[slug][date] = { views: 0, clickThroughs: 0 };
  ...
  async (req, res) => {
    try {
      ...
      const applyStoreFilter = (rows: any[]) =>
        storeFilter
          ? (rows || []).filter((row: any) => row.storeSlug === storeFilter)
          : rows;
      ...
      try {
        ...
        const snapshot = await fetchCMS(
          '*[_type=="complianceSnapshot" && _id == $id][0]',
          {
            id: snapshotId,
          },
        );
...
adminRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const results = await computeCompliance(types, {
        org: targetOrg,
        brand: targetBrand,
      });
      const client = createWriteClient();
      ...
      const runBy =
        (admin && (admin.email || admin.id || admin.name)) || "unknown";
      ...
      await client
        .create(historyDoc)
        .catch(() => client.createOrReplace(historyDoc));
      ...
      try {
        ...
        const existing = await fetchCMS(
          '*[_type=="complianceMonitor" && _id == $id][0]',
          {
            id: monitorId,
          },
        );
        ...
        const prev =
          existing && Array.isArray((existing as any).lastRuns)
            ? (existing as any).lastRuns
            : [];
        const entry: any = {
          scope: targetBrand
            ? `brand:${targetBrand}`
            : `org:${targetOrg || "global"}`,
      ...
      if (err instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: "INVALID_COMPLIANCE_REQUEST", details: err.issues });
      }
  ...
  async (req, res) => {
    try {
      ...
      const limit = Math.max(
        1,
        Math.min(200, Number((req.query as any).limit || 50)),
      );
      // scope: if brand query is provided, enforce RBAC similar to snapshot POST
      let targetOrg = admin.organizationSlug;
      let targetBrand = admin.brandSlug;
      if (admin.role === "OWNER" && req.query?.org)
        targetOrg = String(req.query.org);
      if (req.query?.brand) {
        if (admin.role !== "OWNER" && admin.role !== "BRAND_ADMIN")
          return res.status(403).json({ error: "FORBIDDEN" });
        if (
          admin.role === "BRAND_ADMIN" &&
          String(req.query.brand) !== admin.brandSlug
        )
          return res.status(403).json({ error: "FORBIDDEN" });
  ...
  async (req, res) => {
    try {
      ...
      if (err instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: "INVALID_SETTINGS", details: err.issues });
      }
...
adminRouter.post("/upload-logo", requireRole("EDITOR"), async (req, res) => {
  try {
    ...
    if (!brandResult.ok)
      return res.status(brandResult.status).json({ error: brandResult.error });
    const scopedBrand = brandResult.brand;
    req.log.info("admin.upload_logo", {
      brand: scopedBrand,
      admin: (req as any).admin?.email,
    });
    const client = createWriteClient();
    ...
    const base64 =
      typeof data === "string" ? data.replace(/^data:.*;base64,/, "") : "";
    ...
    const url =
      (uploaded && (uploaded.url || (uploaded.asset && uploaded.asset.url))) ||
      null;
    const assetId =
      uploaded && (uploaded._id || (uploaded.asset && uploaded.asset._id));
    res.json({ ok: true, url, assetId, uploaded });
  } catch (err) {
    if (err instanceof z.ZodError) {
      return res
        .status(400)
        .json({ error: "INVALID_FILE_PAYLOAD", details: err.issues });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (err instanceof z.ZodError) {
        return res
          .status(400)
          .json({ error: "INVALID_FILE", details: err.issues });
      }
...
adminRouter.get("/banner", async (req: any, res) => {
  ...
  try {
    const geoRes = await fetchFn(
      `https://ipapi.co/${encodeURIComponent(clientIp || "0.0.0.0")}/json/`,
    );
    if (geoRes?.ok) {
...
adminRouter.post("/preferences/dashboard", (req: any, res) => {
  ...
  if (!parsed.success)
    return res
      .status(400)
      .json({ error: "INVALID_LAYOUT", details: parsed.error.issues });
...
// --- Control plane scaffolding (tenants, stores, themes, system behavior) ---
const tenantSchema = z.object({
  ...
  slug: z
    .string()
    .regex(/^[a-z0-9-]+$/, "Slug must be lowercase alphanumerics and dashes"),
...
adminRouter.post(
  ...
  async (req, res) => {
    ...
    if (!parsed.success)
      return res
        .status(400)
        .json({ error: "INVALID_TENANT", details: parsed.error.issues });
    try {
      const saved = await upsertTenant(parsed.data, {
        actorId: (req as any).admin?.id,
        expectedVersion: parsed.data.version,
      });
      res.json(saved);
    } catch (err: any) {
      if (err?.message === "TENANT_SLUG_CONFLICT")
        return res.status(409).json({ error: "TENANT_SLUG_CONFLICT" });
      if (err?.message === "VERSION_CONFLICT")
        return res.status(409).json({ error: "VERSION_CONFLICT" });
      req.log.error("control.tenants.save_failed", err);
      res.status(500).json({ error: "FAILED" });
    }
  },
);
...
adminRouter.delete(
  ...
  requireRole("ORG_ADMIN"),
  async (req, res) => {
    const ok = await deleteTenant(req.params.id, (req as any).admin?.id);
...
adminRouter.get("/control/stores", requireRole("VIEWER"), async (req, res) => {
  const tenantId = (req.query?.tenantId as string) || undefined;
  const items = await listStores(tenantId);
  res.json({ items });
});

adminRouter.post("/control/stores", requireRole("EDITOR"), async (req, res) => {
  const parsed = storeSchema.safeParse(req.body || {});
  if (!parsed.success)
    return res
      .status(400)
      .json({ error: "INVALID_STORE", details: parsed.error.issues });
  try {
    const saved = await upsertStore(parsed.data, {
      actorId: (req as any).admin?.id,
      expectedVersion: parsed.data.version,
    });
    res.json(saved);
    ...
    if (err?.message === "TENANT_SUSPENDED")
      return res.status(409).json({ error: "TENANT_SUSPENDED" });
    if (err?.message === "STORE_SLUG_CONFLICT")
      return res.status(409).json({ error: "STORE_SLUG_CONFLICT" });
    if (err?.message === "VERSION_CONFLICT")
      return res.status(409).json({ error: "VERSION_CONFLICT" });
    req.log.error("control.stores.save_failed", err);
    res.status(500).json({ error: "FAILED" });
  }
});
...
adminRouter.delete(
  ...
  requireRole("EDITOR"),
  async (req, res) => {
    const ok = await deleteStore(req.params.id, (req as any).admin?.id);
    if (!ok) return res.status(404).json({ error: "NOT_FOUND" });
    res.json({ ok: true });
  },
);
...
const behaviorSchema = z.object({
  ...
  personalization: z.object({
    strategy: z.enum(["deals-first", "education-first", "hybrid"]),
    weightPurchaseHistory: z.number(),
    weightBrowsing: z.number(),
    weightContext: z.number(),
  }),
  legal: z.object({ reacceptOnUpdate: z.boolean(), ageGate: z.number() }),
  caching: z.object({ aggressive: z.boolean(), previewMode: z.boolean() }),
});

adminRouter.get(
  "/control/behavior/:tenantId",
  requireRole("VIEWER"),
  async (req, res) => {
    const tenantId = req.params.tenantId;
    const existing = await getBehavior(tenantId);
...
adminRouter.post(
  ...
  async (req, res) => {
    const tenantId = req.params.tenantId;
    const parsed = behaviorSchema.safeParse({ ...req.body, tenantId });
    if (!parsed.success)
      return res
        .status(400)
        .json({ error: "INVALID_BEHAVIOR", details: parsed.error.issues });
    try {
      const saved = await saveBehavior(parsed.data, {
        actorId: (req as any).admin?.id,
        expectedVersion: parsed.data.version,
      });
      res.json(saved);
      ...
      if (err?.message === "VERSION_CONFLICT")
        return res.status(409).json({ error: "VERSION_CONFLICT" });
      req.log.error("control.behavior.save_failed", err);
      res.status(500).json({ error: "FAILED" });
    }
  },
);

const themeControlSchema = z.object({
  tenantId: z.string(),
  version: z.number().optional(),
  ...
  assets: z
    .object({ logoUrl: z.string().optional(), iconUrl: z.string().optional() })
    .optional(),
});

adminRouter.get(
  "/control/theme/:tenantId",
  requireRole("VIEWER"),
  async (req, res) => {
    const tenantId = req.params.tenantId;
    const storeId = (req.query?.storeId as string) || null;
    const found = await getTheme(tenantId, storeId);
...
adminRouter.post(
  ...
  async (req, res) => {
    const tenantId = req.params.tenantId;
    const parsed = themeControlSchema.safeParse({ ...req.body, tenantId });
    if (!parsed.success)
      return res
        .status(400)
        .json({ error: "INVALID_THEME", details: parsed.error.issues });
    try {
      ...
      if (err?.message === "VERSION_CONFLICT")
        return res.status(409).json({ error: "VERSION_CONFLICT" });
      req.log.error("control.theme.save_failed", err);
      res.status(500).json({ error: "FAILED" });
    }
  },
);
