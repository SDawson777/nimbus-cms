...
type Store = {_id: string; slug: string; stateCode?: string | null; title?: string}
...
export async function computeCompliance(
  ...
  const params: any = {}
  if (opts?.brand) {
    storeFilter += ' && references(*[_type=="brand" && slug.current==$brand]._id)'
    params.brand = opts.brand
  }
  if (opts?.org) {
    storeFilter += ' && references(*[_type=="organization" && slug.current==$org]._id)'
    params.org = opts.org
  }
  ...
  for (const d of legalDocs || []) {
    ...
    if (!Number.isNaN(vA) && !Number.isNaN(vB)) {
      ...
      const aTime = existing.effectiveFrom ? Date.parse(existing.effectiveFrom) : 0
  ...
  for (const s of stores || []) {
    ...
    const currentLegal: Array<{type: string; version?: string | number; effectiveFrom?: string}> =
      []
    for (const t of requiredTypes) {
      ...
      if (chosen) {
        currentLegal.push({type: t, version: chosen.version, effectiveFrom: chosen.effectiveFrom})
      } else {
        missing.push(t)
      }
    }
