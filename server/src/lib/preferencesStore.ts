...
export function getDashboardLayout(adminId: string | null | undefined): DashboardLayout {
  if (!adminId) return defaultLayout
  return layoutStore.get(adminId) || defaultLayout
}

export function saveDashboardLayout(adminId: string | null | undefined, layout: DashboardLayout) {
  const normalized: DashboardLayout = {
    order: Array.from(new Set(layout.order.length ? layout.order : defaultLayout.order)),
