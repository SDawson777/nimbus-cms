...
async function main() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const media = [
      { url: `https://picsum.photos/seed/${slug}/800/600`, alt: name },
    ];
  ...
  for (let i = 0; i < 15; i++) {
    ...
    await prisma.contentPage.upsert({
      where: { id: `${tenant.slug}-${slug}` },
      update: {
        title: `Greenhouse Article ${i + 1}`,
        body: `Rich content for article ${i + 1}`,
      },
      create: {
        id: `${tenant.slug}-${slug}`,
        tenantId: tenant.id,
  ...
  for (let i = 0; i < legalTypes.length; i++) {
    ...
    await prisma.legalDocument.upsert({
      ...
      update: {
        type: legalTypes[i],
        version: "1",
        body: `${legalTypes[i]} v1`,
      },
      create: {
        id: `${tenant.slug}-legal-${legalTypes[i]}-1`,
        tenantId: tenant.id,
        type: legalTypes[i],
        version: "1",
        body: `${legalTypes[i]} v1`,
