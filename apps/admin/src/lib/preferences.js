...
export async function fetchDashboardLayout() {
  if (!apiBaseUrl())
    return { layout: DEFAULT_LAYOUT, defaults: DEFAULT_LAYOUT };
  ...
  return {
    ...
    defaults: data.defaults || DEFAULT_LAYOUT,
  };
}
...
export async function fetchNotificationPrefs() {
  ...
  return {
    preferences: data.preferences || DEFAULT_NOTIFICATION_PREFS,
    defaults: data.defaults || DEFAULT_NOTIFICATION_PREFS,
  };
}
