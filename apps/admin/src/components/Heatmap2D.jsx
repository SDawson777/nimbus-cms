...
export default function Heatmap2D({stores = [], token}) {
  const center = useMemo(() => {
    ...
    const lon = stores.reduce((sum, s) => sum + (s.longitude || 0), 0) / stores.length
    const lat = stores.reduce((sum, s) => sum + (s.latitude || 0), 0) / stores.length
  ...
  return (
    <div className="heatmap-card">
      <div
        ...
        style={{
          backgroundImage: bgUrl ? `url(${bgUrl})` : 'linear-gradient(135deg,#0b1224,#0f1a33)',
        }}
