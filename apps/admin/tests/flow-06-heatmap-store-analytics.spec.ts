...
test('UX Flow 6: Heatmap & Store Analytics', async ({ page }) => {
  ...
  // Try clicking first marker/store if available
  const clickableMarker = page.locator('.leaflet-marker-icon, .store-marker, button, a').first();
  if (await clickableMarker.count() > 0) {
    await clickableMarker.click().catch(() => console.log('Could not click marker'));
    await page.waitForTimeout(1000);
    await page.screenshot({ path: '/tmp/flow6-store-details.png', fullPage: true });
  }
  ...
  // Verify heatmap page loaded (either map or placeholder)
  const hasHeatmapContent = mapContainer > 0 || storeList > 0 || 
